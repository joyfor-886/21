...
async def create_project(project: CreateProjectRequest, db: Database = Depends(get_db)):
...
async def get_messages(project_id: str, db: Database = Depends(get_db)):
...
async def get_project_stats(project_id: str, db: Database = Depends(get_db)):
...
async def list_local_files(project_id: str, db: Database = Depends(get_db)):
    try:
...
async def get_local_file_content(project_id: str, filename: str, db: Database = Depends(get_db)):
    try:
        project_row = db.fetchone("SELECT name FROM projects WHERE id = ?", (project_id,))
        if not project_row:
...
async def create_project_from_template(template_id: str, db: Database = Depends(get_db)):
