...
class TanyinState:
    def __init__(self, project_id: str):
        ...
        self.current_stage = TanyinStage.INITIAL
        self.planning_phase = PlanningPhase.TANYIN
        self.round = 0
        self.max_rounds = 3
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "TanyinState":
        ...
        state.current_stage = TanyinStage(data.get("current_stage", "initial"))
        state.planning_phase = PlanningPhase(data.get("planning_phase", "tanyin"))
...
class TanyinService:
    _instances: Dict[str, TanyinState] = {}
    ...
    def _ensure_loaded(cls, project_id: str):
        ...
        if project_id not in cls._instances:
            ...
            if data is not None:
                cls._instances[project_id] = TanyinState.from_dict(data)
            else:
                cls._instances[project_id] = TanyinState(project_id)
    ...
    @classmethod
    def get_state(cls, project_id: str) -> TanyinState:
    ...
    @classmethod
    def should_trigger_tanyin(cls, project_id: str, message: str) -> Dict[str, Any]:
        state = cls.get_state(project_id)

        if state.completed:
            return {"should_trigger": False, "reason": "already_completed"}

        if state.planning_phase != PlanningPhase.TANYIN:
        ...
        if state.round > 0:
            return {"should_trigger": True, "reason": "continue_tanyin"}

        return {"should_trigger": False, "reason": "no_trigger"}

    @classmethod
    def get_tanyin_prompt(cls, project_id: str, message: str) -> str:
        state = cls.get_state(project_id)
        ...
        stage_prompts = {
            TanyinStage.INITIAL: cls._get_initial_stage_prompt,
            TanyinStage.EXPANSION: cls._get_expansion_stage_prompt,
            TanyinStage.CLARIFICATION: cls._get_clarification_stage_prompt,
        }
    ...
    @classmethod
    def _get_initial_stage_prompt(cls, message: str, state: TanyinState) -> str:
...
只输出JSON数组，不要其他内容。"""
    ...
    @classmethod
    def _get_expansion_stage_prompt(cls, message: str, state: TanyinState) -> str:
...
只输出JSON数组，不要其他内容。"""
    ...
    @classmethod
    def _get_clarification_stage_prompt(cls, message: str, state: TanyinState) -> str:
        collected_context = cls._format_collected_info(state)
...
只输出JSON数组，不要其他内容。"""
    ...
    @classmethod
    def _format_collected_info(cls, state: TanyinState) -> str:
        lines = []
    ...
    @classmethod
    def _get_uncovered_dimensions(cls, state: TanyinState) -> List[str]:
        return [
            dim
    ...
    @classmethod
    def should_complete_tanyin(cls, project_id: str) -> bool:
        state = cls.get_state(project_id)
    ...
    @classmethod
    def complete_tanyin(cls, project_id: str):
        state = cls.get_state(project_id)
        ...
        try:
            ...
            IntentAnalyzer.update_state(project_id, {
                "tanyin_completed": True,
                "analysis_completed": True,
            })
        except Exception as e:
            logger.warning(f"Failed to sync tanyin state to IntentAnalyzer: {e}")
...
请确保分析具有深度和洞察力，不要只是重复用户说的话。"""
    ...
    def get_summary(cls, project_id: str) -> str:
        ...
        lines.append(f"\n- 探隐轮次: {state.round}/{state.max_rounds}")
